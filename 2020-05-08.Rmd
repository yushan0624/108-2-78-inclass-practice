...
pick_fromTaipeiMetro <- 
  example$residence %in% set_TaipeiMetro   #住大台北
```
...
# 目標是前面的唧唧
target_regex <- "唧唧"
pattern1 <- glue::glue("^{target_regex}")
print(pattern1)
str_view(phrase, pattern1) 
```

```{r}
# 目標是後面的唧唧
target_regex2 <- "唧唧"
pattern2 <- glue::glue("{target_regex2}$")
print(pattern2)
str_view(phrase, pattern2) 
```

##前後鄰居描述Look around:
target後面是B：target_regex(?=B_regex)
...
# 替換民國xx年的xx成西元年數字(針對字串)
str_replace(  
  twDate,
  regex_pattern,  # 要換掉的文字
  as.character(westernYear) # 要替換的內容(要換成字串才能交換)
) -> twDate_reformat
